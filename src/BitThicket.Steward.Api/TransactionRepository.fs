...
module TransactionRepository =
    ...
    let getAsync (factory: IDbConnectionFactory) (tenantContext: TenantContext) (id: Guid) =
        task {
            ...
            cmd.CommandText <-
                """SELECT id, tenant_id, account_id, occurred_at, posted_at, amount_minor, currency,
                          description, merchant, memo, category_id, source, external_id,
                          matched_transaction_id, transfer_account_id, status, match_confidence,
                          sync_event_id, created_at, updated_at
        ...
        task {
            ...
            cmd.CommandText <-
                """UPDATE transactions SET
                       account_id = $1, occurred_at = $2, posted_at = $3, amount_minor = $4,
                       currency = $5, description = $6, merchant = $7, memo = $8,
                       category_id = $9, source = $10, external_id = $11,
                       matched_transaction_id = $12, transfer_account_id = $13,
                       status = $14, match_confidence = $15, sync_event_id = $16,
                       updated_at = $17
                   WHERE id = $18"""
    ...
    let create (factory: IDbConnectionFactory) (accessor: ITenantContextAccessor) : ITransactionRepository =
        ...
        { new ITransactionRepository with
            ...
            member _.CreateAsync(transaction) = createAsync factory transaction
            member _.UpdateAsync(transaction) = updateAsync factory transaction

        ...
        task {
            ...
            | Some tenantContext ->
                ...
                | _ ->
                    ...
                    let! catReader = catCmd.ExecuteReaderAsync()
                    use catReader = catReader
                    let categories = ResizeArray<Category>()
                    while! catReader.ReadAsync() do
                        categories.Add({
                    ...
                    let! txnReader = txnCmd.ExecuteReaderAsync()
            ...
            | Some budget ->
                ...
                | Some _period ->
                    ...
                    for alloc in allocs do
                        ...
                        let places =
                            match budget.CurrencyCode.ToUpperInvariant() with
                            | "BTC" -> 8
                            | _ -> 2
                        let factor = pown 10m places

                        let toMinorUnits (amount: decimal) = int64 (Decimal.Round(amount * factor))

                        Csv.writeRow writer [
                            categoryNames |> Map.tryFind alloc.CategoryId |> Option.defaultValue "Unknown"
                            toMinorUnits allocated |> string
                            toMinorUnits spentDisplay |> string
                            toMinorUnits remaining |> string
                            toMinorUnits rollover |> string
                            budget.CurrencyCode

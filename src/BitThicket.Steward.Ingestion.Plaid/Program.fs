...
open BitThicket.Steward.Ingestion.Plaid
...
let healthHandler : HttpHandler = fun ctx ->
    Response.ofJson {| status = "ok"; version = version; service = "plaid-ingestion" |} ctx

let syncTriggerHandler (client: IPlaidClient) (http: HttpClient) (logger: ILogger) : HttpHandler = fun ctx ->
    task {
        ...
        // 1. Fetch credentials from Core API vault
...
wapp.UseRouting()
    .UseFalco([
        ...
        post "/sync-trigger" (requireServiceToken (fun ctx ->
            let client = ctx.RequestServices.GetRequiredService<IPlaidClient>()
            let http = ctx.RequestServices.GetRequiredService<HttpClient>()
            let logger = ctx.RequestServices.GetRequiredService<ILoggerFactory>().CreateLogger("PlaidSync")
